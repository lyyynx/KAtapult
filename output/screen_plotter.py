...
class ScreenPlotter(OutputDevice):
    ...
    def draw_rectangle(
        ...
        self.screen = cv2.rectangle(self.screen, top_left, bottom_right, [0], 4)
        cv2.imshow("KAtapult", self.screen)
        cv2.waitKey(1)

    def draw_circle(self, center: tuple[int, int], radius: int) -> None:
        self.screen = cv2.circle(self.screen, center, radius, [0], 4)
        cv2.imshow("KAtapult", self.screen)
        cv2.waitKey(1)
    ...
    def draw_path(self, path: list[tuple[int, int]]) -> None:
        for i in range(0, len(path), 20):
            ...
            self.screen = cv2.line(self.screen, point_a, point_b, [0], 4)
